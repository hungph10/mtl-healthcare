                                            ...
                                            for kernel_size in "${KERNEL_SIZE[@]}"; do
                                                for num_heads in "${NUM_HEADS[@]}"; do
                                                    for task in "${TASKS[@]}"; do
                                                        ...
                                                        then
                                                            short_name_task=$task
                                                            EXP_NAME="$short_name_task-$network-seed_$seed-batch_size_$batch_size-epochs_$epochs-p_dropout_$p_dropout-hidden_size_conv_$hidden_size_conv1-$hidden_size_conv2-$hidden_size_conv3-kernel_size_$kernel_size-num_heads_$num_heads-lr_$lr-lr_scheduler_$lr_scheduler" 
                                                            echo "Running: $EXP_NAME"
                                                            python train.py \
                                                                --seed $seed \
                                                                --task $task\
                                                        ...
                                                        then
                                                            ...
                                                            for w_classify in "${W_CLASSIFY[@]}"; do
                                                                for w_regression in "${W_REGRESSION[@]}"; do 
                                                                    EXP_NAME="$short_name_task-$network-seed_$seed-batch_size_$batch_size-epochs_$epochs-p_dropout_$p_dropout-hidden_size_conv_$hidden_size_conv1-$hidden_size_conv2-$hidden_size_conv3-kernel_size_$kernel_size-num_heads_$num_heads-lr_$lr-lr_scheduler_$lr_scheduler-w_regression_$w_regression-w_classify_$w_classify" 
                                                                    echo "Running: $EXP_NAME"
                                                                    python train.py \
                                                                        --seed $seed \
                                                                        --task $task\
